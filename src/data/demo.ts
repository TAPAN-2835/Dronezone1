...
function buildTimeline(status: JobStatus, createdAt: string): JobTimelineEvent[] {
  const base = [{ id: "t1", label: "Request Submitted", timestamp: createdAt, description: "Customer submitted service request" }];
  if (status === "new") return base;
  const events: JobTimelineEvent[] = [
    ...base,
    { id: "t2", label: "Request Accepted", timestamp: createdAt, description: "Provider accepted the job" },
  ];
  if (["en_route", "on_site", "in_progress", "testing", "completed"].includes(status)) {
    events.push({ id: "t3", label: "Engineer En Route", timestamp: createdAt, description: "Assigned engineer dispatched" });
  }
  if (["on_site", "in_progress", "testing", "completed"].includes(status)) {
    events.push({ id: "t4", label: "On Site", timestamp: createdAt, description: "Engineer arrived at location" });
  }
  if (["in_progress", "testing", "completed"].includes(status)) {
    events.push({ id: "t5", label: "Repair In Progress", timestamp: createdAt, description: "Diagnostic and repair started" });
  }
  if (["testing", "completed"].includes(status)) {
    events.push({ id: "t6", label: "Testing", timestamp: createdAt, description: "Quality check and flight test" });
  }
  if (status === "completed") {
    events.push({ id: "t7", label: "Completed", timestamp: createdAt, description: "Job marked complete" });
  }
  if (status === "rejected") {
    return [{ id: "t1", label: "Request Submitted", timestamp: createdAt }, { id: "t2", label: "Request Rejected", timestamp: createdAt, description: "Provider declined this request" }];
  }
  return events;
}
...
export const jobs: Job[] = Array.from({ length: 24 }).map((_, i) => {
  ...
  return {
    ...
    assignedEngineer: isActive || status === "completed" ? engineers[i % engineers.length] : undefined,
...
export const notifications: AppNotification[] = [
  { id: "n1", category: "request", title: "New job request REQ-1026", body: "Koramangala, Bengaluru", time: "10 min ago", read: false, href: "/app/requests/REQ-1026", relatedId: "REQ-1026" },
  { id: "n2", category: "request", title: "Quotation accepted", body: "Rohit Verma accepted quotation for REQ-1024", time: "45 min ago", read: false, href: "/app/jobs/REQ-1024", relatedId: "REQ-1024" },
  { id: "n3", category: "system", title: "New message from Rohit Verma", body: "I've shared the drone location.", time: "1 hr ago", read: false, href: "/app/chat" },
  { id: "n4", category: "payment", title: "Payment received for REQ-1010", body: "₹3,300 credited to your account", time: "Yesterday", read: true, href: "/app/jobs/REQ-1010", relatedId: "REQ-1010" },
  { id: "n5", category: "reminder", title: "Update your availability", body: "Please confirm your working hours for next week", time: "Yesterday", read: true, href: "/app/settings" },
  { id: "n6", category: "payment", title: "Payout processed", body: "Weekly payout of ₹18,420 has been sent", time: "2 days ago", read: true, href: "/app/profile" },
  { id: "n7", category: "grievance", title: "Grievance update GRV-1005", body: "Admin responded to your grievance", time: "3 days ago", read: true, href: "/app/grievances/new", relatedId: "GRV-1005" },
];
