...
function RequestReview() {
  ...
  const requestedDate = job.requestedCompletionDate ? new Date(job.requestedCompletionDate) : new Date(job.scheduledAt);
        ...
        <div className="space-y-4 lg:col-span-2">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground mb-2">Customer Images</div>
                <div className="grid grid-cols-3 gap-2">
                  {(job.attachments ?? []).filter(a => a.type === "image").map((a) => (
                    <div
                      key={a.id}
                    ...
                    >
                      ...
                      <span className="mt-1 text-[10px] font-medium text-muted-foreground">{a.name}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="rounded-lg bg-primary/5 border border-primary/10 p-3">
                ...
                <div className="text-[10px] text-muted-foreground mt-0.5">End of day assumption</div>
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="border-primary/20">
              ...
              <CardContent className="space-y-5">
                ...
                <div className="rounded-lg border bg-muted/30 p-4 space-y-3">
                  <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">More Days Required</div>
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-center">
                      <div className="font-display text-3xl font-bold text-primary">{additionalDays}</div>
                      <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Additional days</div>
                    </div>
                    <Button
                      variant="outline"
                      size="icon"
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 text-sm">
                    <div>
                      ...
                      <div className="font-medium">{requestedDate.toLocaleDateString("en-IN", { dateStyle: "medium" })}</div>
                    </div>
                    <div>
                      <span className="text-xs text-muted-foreground">Proposed Date</span>
                      <div className="font-medium text-primary">{proposedDate.toLocaleDateString("en-IN", { dateStyle: "medium" })}</div>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Timeline Decision</div>
                  <div className="grid gap-2 sm:grid-cols-2">
                    ...
                    >
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-[10px] opacity-70">Complete by customer's requested date</div>
                      </div>
                    </Button>
                    <Button
                    ...
                    >
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-[10px] opacity-70">Submit revised date for approval</div>
                      </div>
                    </Button>
                  </div>
          ...
          {isNew && (
            <Card className="border-primary/20">
              <CardHeader className="cursor-pointer" onClick={() => setShowQuotation(!showQuotation)}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="rounded-lg border bg-muted/30 p-4 space-y-2 text-sm">
                    <div className="flex justify-between text-muted-foreground">
                      <span>Hardware Cost</span><span className="font-medium text-foreground">{inr(hardware)}</span>
                    </div>
                    <div className="flex justify-between text-muted-foreground">
                      <span>Labor Charge</span><span className="font-medium text-foreground">{inr(labor)}</span>
                    </div>
                    <div className="flex justify-between text-muted-foreground">
                      <span>Shipping Charge</span><span className="font-medium text-foreground">{inr(shipping)}</span>
                    </div>
                    <div className="border-t pt-2 flex justify-between text-muted-foreground">
                      <span>Subtotal</span><span className="font-medium text-foreground">{inr(subtotal)}</span>
                    </div>
                    {discountPct > 0 && (
                      <div className="flex justify-between text-success">
                        <span>Discount ({discountPct}%)</span><span className="font-medium">− {inr(discountAmount)}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-muted-foreground">
                      <span>GST ({gst}%)</span><span className="font-medium text-foreground">{inr(gstAmount)}</span>
                    </div>
                  ...
                  {!quoteSent ? (
                    ...
                    <div className="space-y-3">
                      <div className="rounded-lg border border-primary/20 bg-primary/5 p-3 text-center">
                        <div className="text-xs uppercase tracking-wide text-primary font-medium">Quotation Sent</div>
                        <div className="text-sm text-muted-foreground mt-1">Waiting for customer response…</div>
                      </div>

                      {/* Demo: simulate customer response */}
                      <div className="rounded-lg border bg-muted/30 p-3 space-y-2">
                        <div className="text-[10px] uppercase tracking-wide text-muted-foreground font-medium">Demo: Simulate Customer Response</div>
                      ...
                      {customerResponse === "changes" && (
                        <div className="rounded-lg border border-warning/30 bg-warning/5 p-3 text-center">
                          <div className="text-xs text-[oklch(0.45_0.15_75)] font-medium">Customer has requested changes</div>
                          <div className="text-[10px] text-muted-foreground mt-1">Revise the quotation above and re-send</div>
                          <Button
                            size="sm"
                            variant="outline"
                            className="mt-2"
