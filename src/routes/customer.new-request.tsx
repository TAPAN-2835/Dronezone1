...
function NewRequest() {
  ...
  return (
    <form
      onSubmit={(e) => { e.preventDefault(); toast.success("Request submitted!"); nav({ to: "/customer/requests" }); }}
    ...
    >
      ...
      <Field label="Problem Description">
        <textarea rows={4} placeholder="Describe the issue…" className="w-full rounded-xl border bg-card p-3 text-sm outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/15" />
      </Field>
      <Field label="Upload Photos / Videos">
        <div className="grid grid-cols-4 gap-2">
          ...
          <button type="button" className="grid aspect-square place-items-center rounded-xl border-2 border-dashed text-muted-foreground hover:border-primary hover:text-primary">
      ...
      <button type="submit" className="h-12 w-full rounded-xl bg-primary text-sm font-semibold text-primary-foreground hover:bg-primary/90">
        Submit Request
      </button>
    </form>
  );
