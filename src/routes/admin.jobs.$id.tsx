  ...
  if (!job) {
    return (
      <div className="rounded-xl border bg-card p-8 text-center">
        ...
        <Button asChild className="mt-4"><Link to="/admin/jobs">Back to jobs</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <Button asChild variant="ghost" size="sm" className="-ml-2">
        <Link to="/admin/jobs"><ArrowLeft className="h-4 w-4" /> All jobs</Link>
      </Button>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{job.status}</span>
            <JobAgeBadge createdAt={job.createdAt} />
            <span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold">{job.category}</span>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <Card>
            <CardHeader><CardTitle className="text-sm">Job Overview</CardTitle></CardHeader>
          ...
          {relatedGrievances.length > 0 && (
            <Card>
              <CardHeader><CardTitle className="text-sm">Related Grievances</CardTitle></CardHeader>
              <CardContent className="divide-y">
                {relatedGrievances.map((g) => (
                  <Link key={g.id} to="/admin/grievances/$id" params={{ id: g.id }} className="flex items-center justify-between py-3 first:pt-0 last:pb-0 hover:text-primary">
                    <div>
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2 text-sm"><Clock className="h-4 w-4" /> Aging</CardTitle></CardHeader>
            <CardContent><JobAgeBadge createdAt={job.createdAt} className="text-sm px-3 py-1" /></CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2 text-sm"><IndianRupee className="h-4 w-4" /> Payment</CardTitle></CardHeader>
            <CardContent className="text-sm font-semibold">{job.paymentStatus}{job.amount ? ` · ${inr(job.amount)}` : ""}</CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2 text-sm"><Shield className="h-4 w-4" /> AMC</CardTitle></CardHeader>
