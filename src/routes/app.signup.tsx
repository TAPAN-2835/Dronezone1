...
function SignUpPage() {
  ...
  const certOptions = ["DGCA Certified Pilot", "DJI Authorized Technician", "ISO 9001 Trained", "FAA Part 107", "EASA Certification"];
      ...
      <div className="relative hidden flex-col justify-between overflow-hidden bg-[oklch(0.22_0.05_264)] p-12 text-white lg:flex">
        ...
        <div className="relative space-y-6">
          ...
          <p className="text-white/70">
            Get verified, start receiving job requests, and grow your drone repair business with DroneZone.
          </p>
          {/* Progress steps */}
          <div className="space-y-3">
            {steps.map((s) => {
              ...
              return (
                <div key={s.id} className={`flex items-center gap-3 ${current ? "text-white" : done ? "text-white/70" : "text-white/30"}`}>
                  <div className={`flex h-8 w-8 items-center justify-center rounded-full border-2 ${
                    done ? "border-[oklch(0.85_0.15_152)] bg-[oklch(0.85_0.15_152)] text-[oklch(0.22_0.05_264)]" :
                    current ? "border-white bg-white/10" : "border-white/20"
                  }`}>
      ...
      <div className="flex min-h-screen items-center justify-center p-6 sm:p-10">
        ...
        >
          ...
          <div className="mt-6 space-y-4">
            {step === 1 && (
              <>
                <FormField icon={User} label="Full Name" required>
                  <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Rahul Sharma" className="h-11 pl-9" required />
                </FormField>
                <FormField icon={Mail} label="Email" required>
                  <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="rahul@email.com" className="h-11 pl-9" required />
                </FormField>
                <FormField icon={Phone} label="Phone" required>
                  <Input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 98765 43210" className="h-11 pl-9" required />
                </FormField>
            ...
            {step === 2 && (
              <>
                <FormField icon={MapPin} label="Address" required>
                  <Textarea value={address} onChange={(e) => setAddress(e.target.value)} placeholder="12, MG Road, Block A" rows={2} className="pl-9" />
                </FormField>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label>City <span className="text-destructive">*</span></Label>
                    <Input value={city} onChange={(e) => setCity(e.target.value)} placeholder="Bengaluru" className="h-11" required />
                  </div>
                  <div className="space-y-1.5">
                    <Label>State <span className="text-destructive">*</span></Label>
                    <Input value={state} onChange={(e) => setState(e.target.value)} placeholder="Karnataka" className="h-11" required />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label>Pincode <span className="text-destructive">*</span></Label>
                  <Input value={pincode} onChange={(e) => setPincode(e.target.value)} placeholder="560001" className="h-11" required />
                </div>
                <div className="border-t pt-4 mt-4">
                  <div className="text-xs uppercase tracking-wide text-muted-foreground mb-3">Optional Business Details</div>
                  <div className="space-y-3">
                    <FormField icon={Building2} label="Business Name">
                      <Input value={businessName} onChange={(e) => setBusinessName(e.target.value)} placeholder="SkyFix Drone Services" className="h-11 pl-9" />
                    </FormField>
                    <div className="space-y-1.5">
                      <Label>GSTIN</Label>
                      <Input value={gstin} onChange={(e) => setGstin(e.target.value)} placeholder="29AAACS1234A1Z5" className="h-11" />
                    </div>
                  </div>
                </div>
              </>
            ...
            {step === 3 && (
              <>
                <div className="space-y-2">
                  <Label>Certifications <span className="text-destructive">*</span></Label>
                    ...
                    {certOptions.map((c) => (
                      ...
                      >
                        <div className={`flex h-5 w-5 items-center justify-center rounded border-2 transition ${
                          ...
                          {certs.includes(c) && <Check className="h-3 w-3 text-primary-foreground" />}
                        </div>
                ...
                <div className="border-t pt-4 mt-4">
                  <div className="text-xs uppercase tracking-wide text-muted-foreground mb-3">Verification Documents</div>
                  <div className="space-y-3">
                    <UploadField label="DGCA Certificate" value={dgca} onChange={setDgca} />
                    <UploadField label="ID Proof (Aadhaar / PAN)" value={idProof} onChange={setIdProof} />
                    <UploadField label="Business Registration (optional)" value={businessDoc} onChange={setBusinessDoc} />
                  </div>
                </div>
              </>
            )}
              ...
              <div className="space-y-4">
                ...
                <ReviewSection title="Certifications">
                  <div className="flex flex-wrap gap-1.5">
                    {certs.map((c) => (
                      <span key={c} className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">{c}</span>
                    ))}
                  </div>
                </ReviewSection>
...
function FormField({ icon: Icon, label, required, children }: { icon: typeof User; label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
