...
const priorityClass = (p: string) =>
  p === "Critical" || p === "High" ? "text-destructive" : p === "Medium" ? "text-warning" : "text-muted-foreground";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search grievances…" value={q} onChange={(e) => setQ(e.target.value)} className="h-10 pl-9" />
        </div>
            ...
            {filtered.map((g) => (
              <tr key={g.id} className="border-t hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3">
                  {g.raisedBy}
                  <span className="ml-1 text-[10px] uppercase text-muted-foreground">({g.raisedByType})</span>
                </td>
                <td className="px-4 py-3">{g.against}</td>
                <td className="px-4 py-3 text-muted-foreground">{g.category}</td>
                <td className={`px-4 py-3 text-xs font-semibold ${priorityClass(g.priority)}`}>{g.priority}</td>
                <td className="px-4 py-3">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${statusClass(g.status)}`}>{g.status}</span>
                </td>
                <td className="px-4 py-3 text-right">
                  <Button variant="outline" size="sm" asChild>
                    <Link to="/admin/grievances/$id" params={{ id: g.id }}>View</Link>
                  </Button>
                </td>
              </tr>
            ))}
      ...
      <div className="space-y-3 sm:hidden">
        {filtered.map((g) => (
          <Link key={g.id} to="/admin/grievances/$id" params={{ id: g.id }} className="block rounded-xl border bg-card p-4 hover:bg-accent/30">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="font-semibold">{g.id}</div>
                <div className="text-xs text-muted-foreground">{g.issue}</div>
              </div>
              <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold ${statusClass(g.status)}`}>{g.status}</span>
            </div>
            <div className="mt-2 text-xs text-muted-foreground">{g.raisedBy} vs {g.against}</div>
          </Link>
        ))}
      </div>
    </div>
