...
function Chat() {
  const [demoStatus, setDemoStatus] = useState<"Draft" | "Submitted" | "Review" | "Quotation Pending" | "Awaiting Customer Approval" | "Active Job" | "Completed" | "Feedback Submitted" | "Closed">("Active Job");

  const isPreActive = ["Draft", "Submitted", "Review", "Quotation Pending", "Awaiting Customer Approval"].includes(demoStatus);
  ...
  return (
    <div className="flex h-[calc(100vh-9rem)] flex-col bg-[oklch(0.97_0.012_255)]">
        ...
        {isPreActive && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-background/50 backdrop-blur-[1px]">
            <div className="max-w-[80%] rounded-xl bg-card p-4 text-center text-sm shadow-sm border">
              Chat will become available once your service request is accepted and moved to Active Jobs.
            </div>
          </div>
        )}
      ...
      <div className="flex items-center gap-2 border-t bg-card p-3">
        <button disabled={isPreActive || isPostActive} className="grid h-9 w-9 place-items-center rounded-full text-muted-foreground hover:bg-accent disabled:opacity-50"><Paperclip className="h-4 w-4" /></button>
        <button disabled={isPreActive || isPostActive} className="grid h-9 w-9 place-items-center rounded-full text-muted-foreground hover:bg-accent disabled:opacity-50"><Camera className="h-4 w-4" /></button>
        <input disabled={isPreActive || isPostActive} placeholder="Type a message…" className="h-10 flex-1 rounded-full border bg-background px-4 text-sm outline-none focus:border-primary/40 disabled:opacity-50" />
        <button disabled={isPreActive || isPostActive} className="grid h-10 w-10 place-items-center rounded-full bg-primary text-primary-foreground disabled:opacity-50"><Send className="h-4 w-4" /></button>
      </div>
    </div>
  );
}