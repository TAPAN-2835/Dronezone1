  ...
  return (
    <div className="space-y-5 px-5 py-5">
      ...
      <div className="flex items-center justify-between rounded-xl border bg-card p-4">
        <div>
          <Label htmlFor="auto-renew" className="text-sm font-semibold">Auto Renewal</Label>
              ...
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {plan.popular && (
                      <span className="rounded-full bg-primary px-2 py-0.5 text-[10px] font-semibold text-primary-foreground">Popular</span>
                    )}
                    {plan.id === amc.planId && (
                      <span className="rounded-full bg-success/15 px-2 py-0.5 text-[10px] font-semibold text-success">Current</span>
                    )}
                  </div>
                  <div className="mt-1 font-display text-xl font-bold">{inr(plan.price)}<span className="text-xs font-normal text-muted-foreground"> / {plan.duration}</span></div>
                  <div className="mt-0.5 text-xs text-muted-foreground">{plan.visits} service visits included</div>
                </div>
      ...
      <div className="space-y-2">
        <Row icon={Calendar} label="Schedule Service" onClick={() => toast.info("Service scheduling opens calendar picker.")} />
        <Row icon={Wrench} label="Raise AMC Ticket" onClick={() => toast.info("AMC ticket created for your active plan.")} />
        <Row icon={FileText} label="AMC Documents" sub="Download Agreement" onClick={() => toast.success("AMC agreement downloaded.")} />
      </div>
      ...
      {!showRenew ? (
        ...
        <div className="rounded-xl border bg-card p-4 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Extend your plan for another 12 months at {inr(14999)}</p>
          <button
          ...
          <button onClick={() => setShowRenew(false)} className="h-8 w-full text-xs text-muted-foreground">Cancel</button>
        </div>
      )}
    </div>
  );
...
function Row({ icon: Icon, label, sub, onClick }: { icon: typeof Calendar; label: string; sub?: string; onClick?: () => void }) {
  return (
    <button onClick={onClick} className="flex w-full items-center gap-3 rounded-xl border bg-card p-3 text-left hover:bg-accent/40">
