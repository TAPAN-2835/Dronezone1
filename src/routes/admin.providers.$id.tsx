  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-2 flex gap-2">
            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${provider.status === "Pending" ? "bg-warning/15 text-[oklch(0.45_0.15_75)]" : provider.status === "Rejected" ? "bg-destructive/15 text-destructive" : provider.status === "In Review" ? "bg-primary/10 text-primary" : "bg-success/15 text-success"}`}>{provider.status}</span>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader><CardTitle className="text-sm">Provider Profile</CardTitle></CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-2 text-sm">
            <div><span className="text-muted-foreground">Full Name: </span>{provider.provider}</div>
            ...
            <div className="sm:col-span-2"><span className="text-muted-foreground">Address: </span>123 Drone Street, {provider.city}, India 500001</div>
            <div className="sm:col-span-2"><span className="text-muted-foreground">Service Areas: </span>{provider.city}, Suburbs</div>
            <div><span className="text-muted-foreground">Experience: </span>4 Years</div>
            <div><span className="text-muted-foreground">Specializations: </span>Repair, Calibration</div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle className="text-sm">Verification Information</CardTitle></CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-2 text-sm">
            <div><span className="text-muted-foreground">Status: </span>{provider.status}</div>
            <div><span className="text-muted-foreground">Submission Date: </span>{provider.submitted}</div>
            <div><span className="text-muted-foreground">Review Date: </span>14 May 2026</div>
            <div className="sm:col-span-2"><span className="text-muted-foreground">Reviewer Notes: </span>All documents verified. Waiting on final background check clearance.</div>
          </CardContent>
        </Card>
          ...
          <CardContent className="grid gap-6 sm:grid-cols-2 text-sm">
            <div>
              ...
              <ul className="space-y-2">
                {providerDocs.filter(d => d.status === "Verified").map((d) => (
            ...
            <div>
              ...
              <ul className="space-y-2">
                {providerDocs.filter(d => d.status !== "Verified").map((d) => (
                  <li key={d.name} className="flex justify-between rounded-lg border border-warning/30 bg-warning/5 p-2">
                    <span>{d.name}</span>
