              ...
              {adminRequests.map((r) => (
                <tr key={r.id} className="border-t hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <Link to="/admin/users/$id" params={{ id: r.userId }} className="hover:text-primary hover:underline">{r.user}</Link>
                  </td>
                  ...
                  <td className="px-4 py-3"><JobAgeBadge createdAt={r.createdAt} /></td>
                  <td className="px-4 py-3 text-right">
                    <Button size="sm" asChild>
                      <Link to="/admin/requests/$id" params={{ id: r.id }}>View Details</Link>
                    </Button>
                  </td>
                </tr>
              ))}
