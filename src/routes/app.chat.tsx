...
import { Search, Paperclip, Send, Smile, Phone, MoreVertical, ArrowLeft, Lock, MessageSquareOff, Eye } from "lucide-react";
    ...
    <Card className="overflow-hidden p-0">
      <div className="grid h-[calc(100vh-220px)] min-h-[520px] grid-cols-1 md:grid-cols-[320px_1fr]">
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              <>
                ...
                {activeThreads.map((t) => (
                  <ThreadItem
                    ...
                    onClick={() => { setActive(t.id); setMobileOpen(true); }}
                  />
                ))}
              </>
            )}
            ...
            {activeThreads.length === 0 && closedThreads.length === 0 && (
              <div className="p-6 text-center">
                ...
                <p className="mt-2 text-xs text-muted-foreground">Chat is available only for active jobs.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className={`${mobileOpen ? "flex" : "hidden md:flex"} flex-col`}>
          ...
          {/* Messages */}
          <div className={`flex-1 space-y-3 overflow-y-auto p-4 ${(isClosed || isPendingFeedback) ? "opacity-60" : ""} bg-[radial-gradient(circle_at_50%_0%,oklch(0.97_0.015_255)_0%,oklch(0.984_0.005_247)_100%)]`}>
            {msgs.map((m, i) => (
              <motion.div
                key={m.id}
          ...
          {isActive ? (
            ...
            <div className="border-t bg-muted/30 p-4 text-center">
              <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                ...
                {isClosed ? "Chat closed — history view only" : isPendingFeedback ? "Waiting for feedback to close chat" : "Chat available only during active jobs"}
              </div>
            </div>
          )}
        </div>
