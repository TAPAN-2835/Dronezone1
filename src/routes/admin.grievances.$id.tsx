    ...
    return (
      <div className="rounded-xl border bg-card p-8 text-center">
        ...
        <Button asChild className="mt-4">
          <Link to="/admin/grievances">Back</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <Button asChild variant="ghost" size="sm" className="-ml-2">
        <Link to="/admin/grievances">
          <ArrowLeft className="h-4 w-4" /> All grievances
        </Link>
      </Button>
            ...
            <CardContent className="space-y-3 text-sm">
              <div>
                <div className="text-xs uppercase text-muted-foreground">
                  Raised by ({g.raisedByType})
                </div>
                <div className="font-semibold">{g.raisedBy}</div>
                {g.raisedById && (
                  <Link
                    to={g.raisedByType === "customer" ? "/admin/users/$id" : "/admin/providers/$id"}
                    params={{ id: g.raisedById }}
            ...
            <CardContent className="text-sm">
              ...
              {g.resolvedAt && (
                <>
                  ...
                  <div>
                    {new Date(g.resolvedAt).toLocaleString("en-IN", {
                      dateStyle: "full",
                      timeStyle: "short",
                    })}
                  </div>
