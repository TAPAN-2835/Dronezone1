...
import { getProviderHistory } from "@/lib/api/provider.server";
...
function History() {
  ...
  const completed = useMemo(
    () =>
      jobs
        .filter((j: any) => j.status === "completed" || j.status === "cancelled" || j.status === "rejected")
        .filter(
          (j: any) =>
            q === "" ||
                  ...
                  <tr key={j.id} className="hover:bg-muted/30">
                    <td className="px-4 py-3">
                      ...
                      <div className="font-mono text-xs text-muted-foreground">
                        {j.service_requests?.request_number} · {j.service_requests?.drones?.model}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="font-medium">{j.service_requests?.users?.first_name} {j.service_requests?.users?.last_name}</div>
                      <div className="text-xs text-muted-foreground">{j.service_requests?.addresses?.city}</div>
                    </td>
