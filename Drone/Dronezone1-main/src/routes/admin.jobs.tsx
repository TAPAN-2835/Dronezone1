...
import { getAdminJobs } from "@/lib/api/admin.server";
          ...
          <tbody>
            {filtered.map((j: any) => (
              <tr key={j.id} className="border-t hover:bg-muted/20">
                <td className="px-4 py-3 font-semibold"><span className="truncate block w-24">{j.id.split("-")[0]}</span></td>
                <td className="px-4 py-3">{j.service_requests?.customer?.first_name} {j.service_requests?.customer?.last_name}</td>
                <td className="px-4 py-3">{j.provider?.first_name} {j.provider?.last_name}</td>
                <td className="px-4 py-3 text-muted-foreground">{j.service_requests?.service_categories?.name}</td>
        ...
        {filtered.map((j: any) => (
          ...
          >
            ...
            <div className="mt-2 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">{j.service_requests?.customer?.first_name} {j.service_requests?.customer?.last_name}</span>
              <JobAgeBadge createdAt={j.created_at} />
            </div>
          </Link>
        ))}
