...
import { getAdminProviders } from "@/lib/api/admin.server";
...
function AdminProvidersPage() {
  ...
  const displayProviders = providers.filter(
    (p: any) => p.users?.email !== "provider@dronezone.com" || p.status !== "Approved",
  );
  ...
  const docsRequiredCount = providers.filter(
    (p: any) => p.status === "Additional Documents Required",
  ).length;
                    ...
                    {displayProviders.map((p: any) => (
                      <tr key={p.id} className="hover:bg-muted/30">
                        <td className="px-4 py-4">
                          <div className="font-medium">{p.users?.first_name} {p.users?.last_name}</div>
                        ...
                        <td className="px-4 py-4 text-muted-foreground">{p.business_name || "—"}</td>
