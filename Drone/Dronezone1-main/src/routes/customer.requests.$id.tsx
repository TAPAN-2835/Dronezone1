...
import { getRequestDetails } from "@/lib/api/customer.server";
...
function Tracking() {
  const { request, assignment } = Route.useLoaderData();
      ...
      <div className="rounded-2xl border bg-gradient-to-br from-card to-accent/30 p-4">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="mt-0.5 text-xs text-muted-foreground">{request.drones?.model || "Unknown Drone"}</div>
          </div>
            ...
            return (
              <li key={stage.key} className="relative flex gap-3">
                ...
                <div className="pb-1">
                  ...
                  {done && <div className="text-xs text-muted-foreground">{active ? "Current" : "Completed"}</div>}
                </div>
              </li>
            );
          })}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <div className="font-semibold">{assignment.provider_profiles?.business_name || "Assigned Provider"}</div>
              </div>
              <div className="text-xs text-muted-foreground">
                Field Engineer · ★ {assignment.provider_profiles?.average_rating || "New"}
              </div>
            </div>
            <a
      ...
      <div className="rounded-2xl border bg-card p-4 flex items-center gap-3">
        ...
        <div>
          ...
          <div className="font-medium">
            {request.addresses ? `${request.addresses.address_line_1}, ${request.addresses.city}` : "No address specified"}
          </div>
        </div>
      </div>
