...
import { getAdminRequests } from "@/lib/api/admin.server";
            ...
            <tbody>
              {activeRequests.map((r: any) => (
                <tr key={r.id} className="border-t hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{r.service_categories?.name}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold ${priorityClass("Medium")}`}>
                      Medium
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <JobAgeBadge createdAt={r.created_at} />
                  </td>
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="font-semibold">{r.id.split("-")[0]}</div>
                <div className="text-xs text-muted-foreground">{r.users?.first_name} {r.users?.last_name}</div>
              </div>
              <span className="shrink-0 rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">
                {r.status}
              </span>
            </div>
            <div className="mt-2 text-xs text-muted-foreground line-clamp-1">{r.service_categories?.name}</div>
