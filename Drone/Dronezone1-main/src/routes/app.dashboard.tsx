...
import { getProviderDashboard, devSelfAssignJob } from "@/lib/api/provider.server";
...
function Dashboard() {
  const { profile, newRequests, activeJobs, stats } = Route.useLoaderData();
  const router = useRouter();
  
  const recentNotifs = notifications.slice(0, 4);
  const unreadNotifs = notifications.filter((n) => !n.read).length;

  const handleDevAssign = () => {
    toast.promise(devSelfAssignJob(), {
      loading: "Finding a draft request to assign to you...",
      success: "Mock request successfully assigned!",
      error: (e) => e.message,
      finally: () => {
        router.invalidate();
      }
  ...
  return (
    <>
      <PageHeader
        title={`Hi, ${profile?.users?.first_name || "Provider"} 👋`}
        description="Here's how your service business is performing today."
        actions={
          <div className="flex gap-2">
            <Button variant="outline" onClick={handleDevAssign}>
              <Wand2 className="mr-2 h-4 w-4" /> Mock Assign
            </Button>
            <Button asChild>
          ...
          <CardContent className="divide-y">
              ...
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-xs font-mono text-muted-foreground">{j.service_requests?.request_number}</span>
                  ...
                  <div className="mt-0.5 truncate text-xs text-muted-foreground">
                    {j.service_requests?.drones?.model} · {j.service_requests?.addresses?.city}
                  </div>
                </div>
            ...
            {newRequests.length === 0 && (
              <div className="py-8 text-center text-sm text-muted-foreground">
                No new requests available. Click "Mock Assign" to test!
              </div>
            )}
          </CardContent>
        </Card>
            ...
            {activeJobs.slice(0, 4).map((j: any) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-xs text-muted-foreground">{j.service_requests?.addresses?.city}</div>
